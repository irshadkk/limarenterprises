...
public interface UserAttendanceDetailsRepository extends CrudRepository<UserAttendanceDetails, Long>{
	...
	@Query(value = "select count(*),employeeCode, name,status from UserAttendanceDetails where status='Present' and month=:month group by employeeCode,name,status")
    public List<Object[]> findDistinctPresentInMonth(@Param("month") String month);
    
    @Query(value = "select count(*),employeeCode, name,status from UserAttendanceDetails where status='1/2Present' and month=:month group by employeeCode,name,status")
    public List<Object[]> findDistinctHalfPresentInMonth(@Param("month") String month);
